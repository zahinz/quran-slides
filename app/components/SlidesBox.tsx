...
import {
  getQuranChapterTranslation,
  getQuranVerseRecitation
} from '../services/api';
import DOMPurify from 'dompurify';
import SlideControls from './SlideControls';
import { VERSES_AUDIO_URL } from '../services/lib';
...
const SlidesBox = ({
  ...
  verseFrom,
  verseTo
}: SlidesBoxProps): React.JSX.Element => {
  ...
  const [selectedLanguage, setSelectedLanguage] =
    useState<LanguageItem>(selectedLng);
  ...
  const filterVerses = (
    ...
    if (verseFrom > verseTo) {
      alert(
        'Selection error - The last verse number must be less than or equal to the first verse number.'
      );
      return;
    }
    ...
    const filtered = [...verses]
      .filter((verse) => verse.verse_key.startsWith(chapterIdentifier))
      .filter((verse2) => {
        const verseNumber = Number(
          verse2.verse_key.replace(chapterIdentifier, '')
        );
  ...
  const fetchVersesTranslation = async (
    chapterId: number,
    verseFrom: number,
    verseTo: number,
    language: Language
    ...
    if (res) {
      const filteredTranslations = res.translations.slice(
        verseFrom - 1,
        verseTo
      );
      setFilteredVerses((prevFiltered) => {
        return prevFiltered.map((v, i) => ({
          ...v,
          translation: filteredTranslations[i]
        }));
      });
    }
  };
  ...
  const fetchVerseAudio = async (verseKey: string, index: number) => {
    ...
    if (res) {
      setFilteredVerses((prevFiltered) => {
        prevFiltered[index].audio = {
          ...res.audio_files[0],
          url: `${VERSES_AUDIO_URL}/${res.audio_files[0].url}`
        };
        return prevFiltered;
      });
    }
  ...
  return (
    ...
    >
              ...
              <span
                dangerouslySetInnerHTML={{
                  __html: DOMPurify.sanitize(
                    (verse as any)[selectedScript.script_text_key]
                  )
                }}
              />
            ) : (
      ...
      <SlideControls
        ...
        onClickAudio={() =>
          fetchVerseAudio(
            filteredVerses[displayedSlideIndex]?.verse_key,
            displayedSlideIndex
          )
        }
